  ...
  <div id="personal-info-display">
    <a-tabs default-active-key="followed" @change="changeTab">
      <a-tab-pane key="followed">
        ...
        <follow-authors-list-content :get-big-cow="getBigCow" :finish="finish" :has-next="hasNext" :data="followData" @refresh="refresh" style="background: #fff;" />
      </a-tab-pane>
      <a-tab-pane key="fan">
        <span slot="tab">
          {{ $t('common.fan') + ' ' + fanTotal }}
        </span>
        <follow-authors-list-content :get-big-cow="getBigCow" :finish="finish" :has-next="hasNext" :data="fanData" @refresh="refresh" style="background: #fff;" />
      </a-tab-pane>
    </a-tabs>
  </div>
</template>
