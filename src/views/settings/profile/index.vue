      ...
      <div class="left">
        <a-form-model ref="ruleForm" :model="ruleForm" :rules="rules" v-bind="layout" label-align="left">
          ...
          <a-form-model-item has-feedback prop="position" :label="$t('common.position')">
            <a-input v-model="ruleForm.position" :suffix="ruleForm.positionNum + '/50'" @change="commonChange" :max-length="50" :placeholder="$t('common.fillInYourPosition')" />
          </a-form-model-item>
          <!-- 公司 -->
          <a-form-model-item has-feedback prop="company" :label="$t('common.company')">
            <a-input v-model="ruleForm.company" :suffix="ruleForm.companyNum + '/50'" @change="commonChange" :max-length="50" :placeholder="$t('common.fillInYourCompany')" />
          </a-form-model-item>
          <!-- 个人主页 -->
        ...
        <a-col class="avatar-col">
          <div class="avatar">
            <div @click="openUploadModal" class="avatar-container">
              ...
              />
              <img v-else src="@/assets/img/default_avatar.png" width="120" style="border-radius: 50%" />
              <div class="avatar-wrapper">
                <a-icon style="font-size: 38px" type="plus-circle" />
                <span style="line-height: 40px">{{ $t('common.clickToChangeAvatar') }}</span>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getUserInfo() {
      userService
        .getUserInfo({ userId: this.ruleForm.userId })
        .then(res => {
          this.spinning = false;
          (this.ruleForm.username = res.data.name), (this.ruleForm.userNameNum = res.data.name.length);
