<template>
  <div class="container">
    <div v-show="list.length" class="message-content">
      <div v-for="(item, index) of list" :key="index" class="message-item">
        <div class="icon">
          <img src="@/assets/img/message/notification.svg" alt="" />
        </div>
        <div class="content">
...
export default {
  ...
  methods: {
    ...
    getMessageList(isLoadMore) {
      ...
      this.finish = false;
      messageService
        .getMessageList({ current: this.current, size: this.size, type: 0, isRead: false })
        /**
