<template>
  <div>
    <div class="user-list" v-for="(item, index) in data.list" :key="index">
      <div class="item">
        <div class="link">
          <img :src="item.picture ? item.picture : require('@/assets/img/default-avatar.png')" alt="" class="avatar" @click="routerUserCenter(item.id)" />
          ...
          <a-button
            class="follow-btn"
            v-if="!item.isFollow"
            @click="updateFollowState(item.id)"
