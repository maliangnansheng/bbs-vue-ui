<template>
  <div>
    ...
    <div class="user-list" v-for="(item, index) in data" v-else :key="index">
      <div class="item">
        <div class="link">
          <img
          ...
          <div class="info-box" style="flex: 1 1 auto; min-width: 0;">
            <a target="_blank" rel="" class="username" @click="routerUserCenter(getBigCow ? item.toUser : item.fromUser)">
          ...
          <a-button class="follow-btn-close" v-if="item.isFollow" @click="updateFollowState(getBigCow ? item.toUser : item.fromUser)">
